...
from apps.neuf_ldap.utils import (create_ldap_user, ldap_update_user_details, create_ldap_automount,
                                  ldap_update_user_groups)

from apps.neuf_auth.ssh import create_home_dir
from dusken.models import GroupProfile, DuskenUser
...
class Command(BaseCommand):
    ...
    def get_dusken_users_diffable(self):
        ...
        for u in dusken_users:
            dusken_groups = list(u.groups.exclude(profile__posix_name='').values_list('profile__posix_name', flat=True))
    ...
    def user_details_in_sync(self, dusken_user, ldap_user):
        for attr in self.DIFF_ATTRIBUTES:
            if dusken_user[attr] != ldap_user[attr]:
                if int(self.options['verbosity']) >= 2:
                    self.stdout.write('{}: {} (Dusken) != {} (LDAP)'.format(
